    ...
    public List<Hint> generateHints(UUID sessionId) {
        ...
        if (isAttacker) {
            ...
            if (turn <= 5) {
                hints.add(new Hint(
                        "EARLY GAME",
                        "REQUEST SOURCING EARLY",
                        "Request Finance and Logistics early in any cities. Sourcing takes 5 turns of transit to compile before you can return to collect them, so initialize them as soon as possible.",
                        turn
                ));
                hints.add(new Hint(
                        "EARLY GAME",
                        "ESTABLISH HIDE-OUTS",
                        "Build safehouses to manage cell heat. Standard safehouses cost $50K (Home) / $150K (Defender territory). Secure safehouses ($100K / $300K) remain completely invisible to sweeps for 5 turns.",
                        turn
                ));
            }
            if (turn >= 6 && turn <= 15) {
                hints.add(new Hint(
                        "MID GAME",
                        "DEPLOY DEFLECTION TECH",
                        "Deploy Decoy CCTV ($20K) or Decoy Satellite ($40K) to lure Defender agents away with fake scan alerts. Use Active Jammers ($30K) in your node to block scans for 3 turns.",
                        turn
                ));
                if (session.isFinanceCollected() && session.isLogisticsCollected()) {
                    hints.add(new Hint(
                            "MID GAME",
                            "EXECUTE HANDOVER MEETING",
                            "All sourcing assets are acquired. Navigate to any hostile target city and click Initiate Handover. You must stay in that city for 3 turns to complete the transfer.",
                            turn
                    ));
                } else {
                    hints.add(new Hint(
                            "MID GAME",
                            "COLLECT ACTIVE SOURCING",
                            "Return to your requested Finance/Logistics cities once the 5-turn transit counter expires to collect resources.",
                            turn
                    ));
                }
            }
            if (turn >= 16) {
                if (session.isHandoverCompleted()) {
                    hints.add(new Hint(
                            "LATE GAME",
                            "OBTAIN BORDER CLEARANCE",
                            "With the handover complete, request Stage 1 (Infiltration) clearance from HQ to cross the border and access target corridors.",
                            turn
                    ));
                }
                if (session.isInfiltrationGoAheadApproved()) {
                    hints.add(new Hint(
                            "LATE GAME",
                            "REQUEST STRIKE AUTHORIZATION",
                            "Reach the primary target city and request Stage 2 (Strike) authorization from HQ, then execute strike when ready.",
                            turn
                    ));
                }
            }
            ...
            if (heat >= 50) {
                hints.add(new Hint(
                        "WARNING",
                        "HIGH DETECTION HEAT WARNING",
                        "Cell detection heat is at " + heat + "% at current position. Defender searches or lockdowns are imminent. Relocate to safehouses or connect adjacent nodes immediately.",
                        turn
                ));
            }
        } else {
            // Defender Hints (turns 1-5)
            if (turn <= 5) {
                hints.add(new Hint(
                        "EARLY GAME",
                        "SECURE A FOOTHOLD",
